...
function loadBestSellingProduct(language) {
  const bestSellingProduct = document.getElementById('product-best-selling');
  bestSellingProduct.textContent = '';
  let index = 1;
  while (index < 4) {
    fetch(`${API_HOST}${language}_products/${index}`)
      .then(res => res.json())
      .then(data_product => {
        getCategoryAndSubCategory(data_product.sub_category_id, language)
          .then(categoryData => {
            const html = `<div class="flex mt-8 border-b border-dashed pb-6 ">
            <img src="${data_product.image}" alt="${data_product.name}" class="mr-4 w-[101px] h-[101px]">
                  <div>
                    <a href="../../src/pages/detail.html?category=${categoryData.category_id}&item=${categoryData.sub_category_id}&id=${data_product.id}" class="mb-3 font-semibold hover:underline">${data_product.name}</a>
...
function loadFrequentlyPurchasedProducts(language) {
